...
def create_set_with_first_sticker(sticker_data, emoji):
    ...
    stickers = [{"sticker": "attach://file_0", "emoji_list": [emoji]}]
    ...
    files = {"file_0": ("sticker.webp", sticker_data, "image/webp")}
...
def add_sticker(sticker_data, emoji):
    ...
    sticker = {"sticker": "attach://file_0", "emoji_list": [emoji]}

    data = {"user_id": USER_ID, "name": PACK_NAME, "sticker": json.dumps(sticker)}
    files = {"file_0": ("sticker.webp", sticker_data, "image/webp")}
...
def main():
    ...
    print(
        "❗️ Обрати внимание: новые эмодзи могут появиться в клиенте не сразу (иногда нужно подождать пару минут или перезапустить приложение)."
    )
