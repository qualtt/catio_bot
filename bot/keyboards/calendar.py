...
def slot_marker(day: date, min_date: date, free_slots: int, max_slots: int) -> Tuple[str, str]:
    # Returns (fallback_text, color_name)
    # Прошедшие дни (до min_date)
    if day < min_date:
        return "⚫️", "black"
    ...
    # Если всего 1 слот (и он свободен), то зеленый
    if max_slots <= 1:
        return "🟢", "green"
    ...
    *,
    ...
    for week in month_weeks:
        for day_number in week:
            ...
            # День кликабелен, если он входит в диапазон дат и есть хотя бы 1 свободный слот
            enabled = min_date <= day <= max_date and free_slots > 0
            callback_data = f"cal_day_{day.isoformat()}" if enabled else "noop"
            
            # Вставляем кастомный эмодзи. Для этого используем текст-заглушку и icon_custom_emoji_id
            custom_emoji_id = EMOJI_IDS.get(color_name, {}).get(day_number)
            if custom_emoji_id:
                # Текст " " (пробел), потому что кастомный эмодзи будет показан ПЕРЕД текстом
                builder.button(
                    text=" ", 
