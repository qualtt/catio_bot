...
async def handle_album_animal_navigation(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    logger.info("Navigating album: current %s, next %s (offset %s)", current_index, next_index, offset)
        ...
        async with async_session() as session:
            ...
            if not is_auto_scheduled:
                ...
                if schedule_time.timetz().replace(tzinfo=None) not in free_times:
                    ...
                    await callback.answer("К сожалению, это время уже занято. Пожалуйста, выберите другое.", show_alert=True)
                    await _render_dashboard(callback, state, bot)
                    return
...
async def handle_dash_submit_album(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    try:
        async with async_session() as session:
            ...
            schedule_times = [datetime.fromisoformat(it["schedule_time"]) if it.get("schedule_time") else None for it in items]
            ...
            if conflict_index is not None:
                ...
                await callback.answer(f"Время для фото {conflict_index + 1} уже занято. Пожалуйста, выберите другое.", show_alert=True)
                await _render_dashboard(callback, state, bot)
                return
...
async def handle_calendar_day(callback: CallbackQuery, state: FSMContext):
    ...
    if _is_album_submission(data):
        ...
        items = _album_items(data)
        index = int(data.get("album_index") or 0)
        selected_slots = {datetime.fromisoformat(it["schedule_time"]) for i, it in enumerate(items) if i != index and it.get("schedule_time")}
...
async def handle_manual_time(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    await _render_dashboard(callback, state, bot)
    await callback.answer()
__all__ = ['handle_album_animal_navigation', 'handle_animal_type', 'handle_animal_type_back', 'handle_calendar_day', 'handle_calendar_nav', 'handle_custom_animal_type_button', 'handle_dash_album_auto_all', 'handle_dash_change_type', 'handle_dash_set_auto', 'handle_dash_set_manual', 'handle_dash_submit', 'handle_dash_submit_album', 'handle_extra_animal_type', 'handle_manual_time', 'handle_other_animal_type', 'logger', 'select_animal_type']
